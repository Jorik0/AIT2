...
This is the report for the Goal Multi-Agent Elevator assignment. In this assignment we were asked to develop a multi-agent system in which a manager agent allocates task among elevators (each controlled by a single agent) using an auction mechanism.
...
This report is sectioned in the same way as the assignment. That is, in part A we will answer some questions posed in relation to the original (unextended) multi-agent system. In part B we describe our extension's design, implementation and performance. Here we will also give our conclusions about the success of the agent and the use of logic in implementing this kind of system. Finally, in part C we give some possible improvements for Goal.
